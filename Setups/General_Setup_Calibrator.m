function [ cmf ] = General_Setup_Calibrator(x,dir)
% setup file for bucket model (test case 1)
...
% dir = 'C:\Users\marcu\OneDrive - University of Arizona\Documents\2D_Boussinesq_Model\Inverse_Problems\Uniform_Hillslope\Unsteady_State\(b)\Input_Data_GW_Model_uniform_b.xlsx';
...
flag_plot = 0;
[general_input,Setup_Name,BC_GW,GW_Ksat,GW_S,BC_GW_Index,GW_IC,Surf_Elevation,Recharge_BC,X,Y] = Read_Excel_Input_GW(dir,flag_plot);
close all
%% General Settings
...
mask = double(mask);
% Ksat,h
gwK = x(1)*ones(size(GW_Ksat));                      % saturated hydraulic conductivity
gwK(end,:) = 100000*x(1);                             % B.C for seepage face
% Sy
% gwSy = x(2)*ones(size(GW_Ksat));                   % specific yield
gwN = x(2)*ones(size(GW_Ksat));                      % V.G N parameter
gwSy = 0.3*ones(size(GW_Ksat));                      % Now is state dependent
gwH = mask*H;                                        % Aquifer Tickness [m]
gwPoro = x(3)*ones(size(GW_Ksat));                   % UZ Porosity [-]
gwAlpha = x(4)*ones(size(GW_Ksat));                  % Alpha of the UZ zone [1/m]
...
gw_k_UZ = x(5)*ones(ny,nx); % UZ damping factor (linear reservoir) [1/sec]
gw_irr_damping = x(6); % Damping factor in irrigation [-]
...
gwmodel = GWModel(z0,lsurf,nx,ny,dx,dy,gwparameter,gwboundaries,wells,rivers,...
    IC_GW,gwsolver,dt_GW,doAdaptiveTime_GW,0,[]);
